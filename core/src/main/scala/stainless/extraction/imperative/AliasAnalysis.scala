...
//
// XXX: work in progress by Regis. Complain to him @regb!
//
//package stainless
//package extraction
//package imperative
...
//    */
//  def functionAbstraction(ft: FunctionType): Seq[Int] = ft.from.indices
//
//
...
//          }
//          case Some(body) => {
//            val freeVars = exprOps.variablesOf(body)
...
//    private val aliases: MutableMap[Variable, Set[Variable]] = MutableMap.empty
//
//    /* add an alias, bidirectional */
//    def addAlias(v: Variable, alias: Variable): Unit = {
//      aliases(v) = aliases.getOrElse(v, Set(v)) + alias
//      aliases(alias) = aliases.getOrElse(alias, Set(alias)) + v
//    }
//
//    def addAliases(aliases: Set[Variable]): Unit = {
//      for (a1 <- aliases; a2 <- aliases) addAlias(a1, a2)
//    }
//
//    def apply(v: Variable): Set[Variable] = aliases.getOrElse(v, Set(v))
//
...
//    expr: Expr, currentAliases: MutableMap[FunDef, Set[Variable]], localAliases: AliasingGraph
//  ): Set[Variable] = {
//
//    def rec(e: Expr): Set[Variable] = e match {
//      //if localAliases contains an id, it automatically maps it to itself at least
//      case v: Variable => localAliases(v)
//      case Let(vd, e, b) =>
//        val eas = rec(e)
//        eas.foreach(ea => localAliases.addAlias(vd.toVariable, ea))
//        rec(b)
//
//      case LetVar(vd, e, b) =>
//        val eas = rec(e)
...
//        val eas = rec(e)
//        val subBinders = cses.flatMap(mc => mc.pattern.binders.map(_.toVariable)).toSet
//        localAliases.addAliases(subBinders) //all binders are aliases to each other
//        eas.foreach(ea => subBinders.foreach(nvd => localAliases.addAlias(ea, nvd)))
...
//        val fd = fi.tfd.fd
//        val params = fd.params.map(_.toVariable)
//        val fdAliases: Set[Variable] = currentAliases.getOrElse(fd, Set())
//        val (paramAliases, externalAliases) = fdAliases.partition(alias => fd.params contains alias)
//        val fiAliases: Set[Variable] = paramAliases.flatMap { fdAlias =>
//          val aliasedArg = args(fd.params.indexOf(fdAlias))
//          rec(aliasedArg)
//        }
...
//      case Assignment(v, e) =>
//        val vAliases = rec(e)
//        vAliases.foreach(alias => localAliases.addAlias(v, alias))
//        Set()
//
//      case Block(es, e) =>
...
//        rec(e)
//
//      case ADT(_, args) => args.toSet.flatMap((arg: Expr) => rec(arg))
//      case ADTSelector(obj, _) => rec(obj)
//      case FiniteArray(elems, base) => elems.flatMap(rec).toSet
//      case LargeArray(elems, default, length, _) =>
//        rec(length)
//        (elems.values.toSeq :+ default).flatMap(rec).toSet
//
