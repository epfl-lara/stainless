...
object FdLibm {
  ...
  @ignore
  def testTan(): Unit = {
    Tan.compute(-5.871316842144255E96)
    var errors = 0
    val r = scala.util.Random
    r.setSeed(123)
    var j: Long = 0
    var i: Long = Int.MinValue
    val s1 = Tan.compute(5.319372648326541E255)
    val s2 = Math.tan(5.319372648326541E255)
    ...
    println(s"[tan] errors, worst-case: $errors")
    errors = 0
    while (j <= 10000000000L) {
      val d = java.lang.Double.longBitsToDouble(r.nextLong())
      if (j % 10000000 == 0)
        println(s"[tan] progress: $j")
      val s1 = Tan.compute(d)
      val s2 = Math.tan(d)
      val diff = Math.abs(s1 - s2)
      val ulp = Math.min(Math.ulp(s1), Math.ulp(s2))
      if diff > ulp then {
        println(s"[tan] error: d = $d, diff = $diff, s1 = $s1, s2 = $s2, ulp(s1) = ${Math.ulp(s1)}, ulp(s2r) = ${Math.ulp(s2)}")
        j = 10000000000L + 1
        i = Int.MaxValue.toLong + 1
        errors += 1
      }
    ...
    println(s"[tan] errors, random doubles: $errors")
    errors = 0
    while (i <= Int.MaxValue) {
      val d = java.lang.Float.intBitsToFloat(i.toInt).toDouble
      if (i % 100000000 == 0)
        println(s"[tan] progress: $d")
      val s1 = Tan.compute(d)
      val s2 = Math.tan(d)
      val diff = Math.abs(s1 - s2)
      val ulp = Math.min(Math.ulp(s1), Math.ulp(s2))
    ...
    println(s"[tan] errors, singles: $errors")
  }
  ...
  def testSin(): Unit = {
    ...
    var j: Long = 0
    var i: Long = Int.MinValue
    val s1 = Sin.compute(5.319372648326541E255)
  ...
  def testCos(): Unit = {
    ...
    var j: Long = 0
    var i: Long = Int.MinValue
    val s1 = Cos.compute(5.319372648326541E255)
