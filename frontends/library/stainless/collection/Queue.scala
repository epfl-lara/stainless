...
object QueueSpecs {
  ...
  def propEnqueue[A](queue: Queue[A], elem: A) : Unit = {
    import Queue._
  ...
  def propFront[A](queue: Queue[A], elem: A) : Unit = {
  ...
  def propTail[A](queue: Queue[A], elem: A) : Unit = {
    require(!queue.isEmpty && queue.isAmortized)
  ...
  def enqueueAndFront[A](queue: Queue[A], elem: A) : Unit = {
    if (queue.isEmpty)
