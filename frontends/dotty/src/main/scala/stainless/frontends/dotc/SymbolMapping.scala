...
class SymbolMapping {
  ...
  /** Returns a mapping from used Tasty file paths to the corresponding root
    * tree.
    *
    ...
    *
    * We key by [[AbstractFile]] (the Tasty file) rather than by tree, because
    * in Scala 3.8+ `rootTree` may return the same tree object for multiple
    * classes defined in files with chained package clauses (e.g.
    * `package stainless; package collection`), which would cause map
    * collisions.
    *
    ...
    * units.
    */
  def popUsedTastyUnits(): LinkedHashMap[AbstractFile, tpd.Tree] =
    val res = usedTastyUnits
    usedTastyUnits = LinkedHashMap[AbstractFile, tpd.Tree]()
    res

  private var usedTastyUnits = LinkedHashMap[AbstractFile, tpd.Tree]()
  ...
  private def maybeRegisterTastyUnit(sym: Symbol)(using Context): Unit = {
    if (sym.tastyInfo.isDefined) {
      ...
      usedTastyUnits += (classSym.compilationUnitInfo.associatedFile -> classSym.rootTree)
    }
  }
