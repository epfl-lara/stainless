...
class CodeExtraction(inoxCtx: inox.Context, cache: SymbolsContext)(implicit val ctx: Context)
  ...
  private def isValidParentType(parent: Type, inLibrary: Boolean = false): Boolean = parent match {
    ...
    case tpe if getResolvedTypeSym(tpe.typeSymbol) == scalaProductClassSym => false
  ...
  private def extractClass(td: tpd.TypeDef)(implicit dctx: DefContext): (xt.ClassDef, Seq[xt.FunDef]) = {
    ...
    val parents = template.parents
      ...
      .map(p => extractType(p.tpe)(tpCtx, p.pos))
      .map {
  ...
  private def extractLocalClassType(tr: TypeRef, cid: Identifier, tps: List[xt.Type])
    ...
    val parents = tr.parentsWithArgs.filter(isValidParentType(_)).map(extractType(_)(tpCtx, pos))
