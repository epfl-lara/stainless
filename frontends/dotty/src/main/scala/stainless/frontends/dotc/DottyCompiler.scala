    ...
    override def apply(ctx: inox.Context, compilerArgs: Seq[String], callback: CallBack): Frontend =
      new ThreadedFrontend(callback, ctx) {
        val args = {
          ...
          val flags = Seq("-Yretain-trees", "-color:never", "-language:implicitConversions", "-language:experimental.qualifiedTypes", "-language:experimental.qualifiedTypes.silent", "-Yqualified-types-anf", "-Wsafe-init", s"-cp:$cps") // -Ysafe-init is deprecated (SAM 21.08.2024)
