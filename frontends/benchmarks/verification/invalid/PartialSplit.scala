// Disabled until we get @partialEval
...
//   val total     = spent.foldLeft(BigInt(0)) { case (acc, S(_, a)) => acc + a }
//   val perPerson = total / spent.length
//   val diff      = spent map { case S(w, a) => S(w, perPerson - a) }
//   val pos       = diff.filter(_.amount >= 0)
//   val neg       = diff.filter(_.amount < 0)
//   val posTotal  = pos.map(_.amount).foldLeft(BigInt(0))(_ + _)
//   val negTotal  = neg.map(_.amount).foldLeft(BigInt(0))(_ + _)
