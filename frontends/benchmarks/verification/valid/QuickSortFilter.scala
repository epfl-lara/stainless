...
object QuickSortFilter {
  ...
  @induct
  def append_sorted(l1: List[BigInt], l2: List[BigInt]): Boolean = {
    require(isSorted(l1) && isSorted(l2) && (l1.isEmpty || l2.isEmpty || l1.last <= l2.head))
    isSorted(l1 ++ l2)
  }.holds

  @induct
  def filter_equal_sorted(ls: List[BigInt], x: BigInt): Boolean = {
    isSorted(ls filter (_ == x))
  }.holds

  def cons_filter_equal_sorted(ls: List[BigInt], x: BigInt): Boolean = {
    assert(filter_equal_sorted(ls, x))
    isSorted(Cons(x, ls filter (_ == x)))
  }.holds

  @induct
  def forall_last(ls: List[BigInt], p: BigInt => Boolean): Boolean = {
    ls.forall(p) ==> (ls.isEmpty || p(ls.last))
  }.holds

  @induct
  def append_preserves_forall(l1: List[BigInt], l2: List[BigInt], p: BigInt => Boolean): Boolean = {
    require(l1.forall(p) && l2.forall(p))
    (l1 ++ l2).forall(p)
  }.holds

  @induct
  def filter_preserves_forall(ls: List[BigInt], p1: BigInt => Boolean, p2: BigInt => Boolean): Boolean = {
    require(ls.forall(p2))
    ls.filter(p1).forall(p2)
  }.holds

  @induct
  def forall_lt_implies_le(ls: List[BigInt], x: BigInt): Boolean = {
    require(ls.forall(_ < x))
    ls.forall(_ <= x)
  }.holds

  @induct
  def forall_eq_implies_le(ls: List[BigInt], x: BigInt): Boolean = {
    require(ls.forall(_ == x))
    ls.forall(_ <= x)
  }.holds
