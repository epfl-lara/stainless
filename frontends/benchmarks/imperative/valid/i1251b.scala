...
object i1251b {
  ...
  def hhhh1(bbbb1: BBBBox, bbbb2: BBBBox, bbbb3: BBBBox, cond1: Boolean, cond2: Boolean, cond3: Boolean, i: Int, j: Int): Unit = {
    ...
    mutate(bbbb1.fff.ff1(j).f, 456)
    assert(cond2 ==> (bbbb3.fff.ff1(j).f.value == b3Old1j))
  }

  def mutateArrReplace(arr: Array[Box], i: Int, v: Int): Unit = {
    require(0 <= i && i < arr.length)
    arr(i) = Box(v)
  }

  def mutateArrModify(arr: Array[Box], i: Int, v: Int): Unit = {
    require(0 <= i && i < arr.length)
    arr(i).value = v
  }
