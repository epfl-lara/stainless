 ...
 */
object TraceInductHelper {
  val deepInduct = true // a flag for enabling deep induction pattern discovery  
  def createTactFun(program: Program, function: FunDef, reporter: Reporter): Option[FunDef] = {
    val cg = program.callGraph
    assert(!cg.isRecursive(function) && function.hasBody && function.hasPostcondition)
    ...
    if (inductFunname.isDefined)
      reporter.debug("Extracting induction pattern from: " + inductFunname.get)(DebugSectionVerification)
    ...
    funInv match {
      case None =>
        reporter.warning("Cannot discover induction pattern!")
        None
      case Some(finv) =>
...
class TraceInductionTactic(vctx: VerificationContext) extends Tactic(vctx) {
  ...
  import TraceInductHelper._
  def generatePostconditions(function: FunDef): Seq[VC] = {
    if (function.hasPostcondition) {
      createTactFun(vctx.program, function, ctx.reporter) match {
        case Some(tactFun) =>
          // print debug info if needed
        ...
        case _ =>
          ctx.reporter.warning("Falling back to normal tactic.")
